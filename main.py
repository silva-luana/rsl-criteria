...
def separate_files():
	df_criteria = pd.read_csv('criterios_inclusao_exclusao.csv')
...
def main():
	...
	try:
		for ix, row in df.iterrows():
			if ix+1 == len(df):
				resp = input('Voce deseja separar os arquivos a incluir e a excluir? (\'s\' ou \'n\')')
				if resp == 's':
					separate_files()
				else:
					print('Bom trabalho! Ate mais..')

			elif ix+1 not in done_ids:
				clear()
